...
| DataDome JavaScript Tag | 协议 primitive | `datadome_js_tag_signals_experimental` | experimental | JS signal request / `datadome` cookie primitive |
...
### Powxy reverse-proxy PoW WAF

Powxy 是反向代理型 PoW 防护，适合 Git forge/API/文档站这类被爬虫高频打的场景。它比普通 `challenge+nonce` PoW 多了一个环境绑定层：服务端先按当前请求生成 32 字节 identifier，再让浏览器/WASM 搜索 nonce。

源码链路（`go.lindenii.runxiyu.org/powxy@v0.1.86`）：

```text
identifier = SHA256(
  go_varint10(unix_week)
  || remote_ip
  || User-Agent
  || Accept-Encoding
  || Accept-Language
...
pow = SHA256(identifier || uint64_le(nonce))
pass iff first N bits are zero
form field: powxy = base64(uint64_le(nonce))
solved cookie: powxy = base64(HMAC-SHA256(privkey, identifier))
```

SDK 当前做法：

- GET protected page，解析 `body[data-identifier][data-difficulty]` 或 `<pre id="identifier">`。
- 全程固定同一组 `User-Agent / Accept-Encoding / Accept-Language`，避免 GET 和 POST 的 identifier 不一致。
- 复现官方 WASM/C/Go/Python solver 的 `uint64_le` nonce 规则，而不是错误地拼十进制字符串。
- POST 同 URL 或显式 `--submit-url`，成功时识别 303 redirect / `powxy` cookie。
- 支持直接传 HTML、base64 identifier、JSON fixture；不启动浏览器。
- 支持 artifact：`powxy_run.json`。
...
```bash
antibot solve powxy \
  --challenge-json '{"identifier":"AAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8=","difficulty":20}' \
  --max-attempts 200000
```
...
```bash
antibot stress powxy \
  --challenge-json '{"identifier":"AAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8=","difficulty":16}' \
  --runs 20 \
  --concurrency 4 \
  --max-attempts 20000
```
...
- `datadome`：experimental provider，先落地 DataDome JavaScript Tag 的 browserless VM primitive：用 Node `vm` 补最小 `window/document/navigator/location/fetch/XMLHttpRequest/sendBeacon/Image/Storage/userAgentData` 环境，执行本地或显式允许获取的 `tags.js`，捕获 `/js/` signal request、headers/body、`document.cookie` 中的 `datadome` 写入；可选 replay 到受控 endpoint，解析 `Set-Cookie`/`x-set-cookie`/JSON body 里的 `datadome` cookie 和 captcha URL。它不启动浏览器，不做滑块/视觉 CAPTCHA，不宣称完整 DataDome Device Check/CAPTCHA 绕过；真实站仍受 tag/VM 版本、`ddjskey`、CNAME/endpoint、cookie/session、TLS/HTTP2 指纹、header order、IP reputation、UA/CH 一致性与服务端策略影响。
...
antibot solve basedflare \
  --base-url https://target.example/protected \
  --submit \
  --difficulty-bits 16

antibot solve acwscv2 \
  --base-url https://target.example/protected \
  --submit
...
antibot solve datadome \
  --script-js @tags.js \
  --page-url https://target.example/ \
  --endpoint-url https://api-js.datadome.co/js/ \
  --raw

antibot solve datadome \
  --script-js @tags.js \
  --page-url https://target.example/ \
  --endpoint-url https://target.example/js/ \
  --submit \
  --submit-url https://target.example/js/ \
  --raw

antibot solve perimeterx \
  --script-js @px.js \
  --page-url https://target.example/ \
  --collector-url https://target.example/api/v2/collector \
  --raw
...
antibot solve vercel_botid \
  --script-js @raw-c.js \
  --raw-vm \
  --submit-url https://target.example/api/contact/test \
  --x-path /api/contact/test \
  --x-method POST \
  --submit \
  --success-contains accepted \
  --raw
```
...
### 20.4 JustNoCaptcha multi-puzzle FNV PoW

JustNoCaptcha 不是图片识别类验证码，而是把一个 challenge 拆成多个 32 hex puzzle，让客户端逐个搜索定长十进制 candidate。SDK 复现浏览器端 `FNV1a + fmix` 的 `hash/hashInt`，因此可以纯协议求解，不需要 headless。

关键点：

```text
challenge = difficulty + puzzlesHex + hash(difficulty+puzzlesHex+challengeSalt)
difficulty = int(challenge[0])  # 1..7
puzzle = 每 32 hex 一段
candidate = first decimal in [10^(difficulty+1), 10^(difficulty+2)] where hashInt(puzzle+candidate) <= 10^(10-difficulty)
solution = concat(candidate per puzzle)
```

SDK 当前支持：

- 解析 challenge 字符串、JSON、HTML hidden input 或 challenge endpoint；
- 可选传入 `challengeSalt` 校验 challenge 末尾 hash，避免解伪造/损坏 challenge；
- 多 puzzle 顺序或进程池并发求解；
- 输出 `{challenge, solution}` 或自定义 hidden field；
- 不启动浏览器。

命令示例：

```bash
antibot solve justnocaptcha \
  --challenge-url 'https://target.example/justnocaptcha/challenge' \
  --timeout 60

antibot solve justnocaptcha \
  --challenge-json '{"challenge":"30123456789abcdef0123456789abcdeffedcba9876543210fedcba987654321000112233445566778899aabbccddeeff75d657ca1816d8d2fdffaaf0c8ef691d","challengeSalt":"randomtestsalt"}' \
  --timeout 5

antibot stress justnocaptcha \
  --challenge-json '{"challenge":"30123456789abcdef0123456789abcdeffedcba9876543210fedcba987654321000112233445566778899aabbccddeeff75d657ca1816d8d2fdffaaf0c8ef691d","challengeSalt":"randomtestsalt"}' \
  --runs 20 \
  --concurrency 4
```
...
- 这是多 puzzle PoW 协议 solver；
- challengeSalt 是服务端秘密，SDK 只在本地 fixture/已知盐场景做完整性校验，不依赖它求解；
- replay/一次性消费由目标服务端保存 challenge 状态决定。
...
### 22.3 PowForge CAPTCHA signed SHA-256 token PoW

PowForge CAPTCHA 是一个 self-hosted PoW CAPTCHA：widget 先从 `/api/challenge` 拉取 `salt/difficulty/signature/id`，Web Worker 本地挖 `SHA256(salt + nonce)`，再把 nonce 和服务端签名 challenge 提交到 `/api/verify` 换业务表单字段 `pf_token`。SDK 当前把这条链路做成纯协议 solver，不需要 headless，也不需要加载 widget。

关键点：

```text
GET /api/challenge
-> {salt, difficulty, algo:"sha256", signature, id}

nonce starts at 1
hash = SHA256(salt + decimal_nonce)
pass = leading_zero_bits(hash) >= difficulty

POST /api/verify {salt, nonce, signature, algo, difficulty, id, challenge:null}
-> {valid:true, token, method:"sha256"}
submit body = {"pf_token": token}
```

SDK 当前支持：

- 解析 live `/api/challenge`、challenge JSON、文件或 inline salt；
- 精确复现 worker 的 nonce 从 `1` 开始递增语义；
- 支持 `--workers` 分段搜索、`--no-submit` 只解 PoW、`--token-verify` 闭环校验 token；
- 输出 `pf_token` hidden field，并保留 verify body/token verify 现场；
- 不启动浏览器，适合 VPS/headless 受限环境。

命令示例：

```bash
antibot solve powforge \
  --base-url 'https://captcha.powforge.dev' \
  --token-verify

antibot solve powforge \
  --challenge-json '{"salt":"powforge-fixture-salt","difficulty":14,"algo":"sha256"}' \
  --no-submit \
  --timeout 5

antibot stress powforge \
  --challenge-json '{"salt":"powforge-fixture-salt","difficulty":14,"algo":"sha256"}' \
  --no-submit \
  --runs 20 \
  --concurrency 4
...
- 这是 signed challenge + SHA-256 leading-zero bit + token verify 协议 solver；
- SDK 完成客户端 PoW，不伪造服务端签名、token secret、Lightning/L402 skip 状态；
- live 模式每轮应重新拉 `/api/challenge`，不要复用一次性 token。
...
### 23.1 NeoIRC resource/body-bound Hashcash

NeoIRC 把传统 Hashcash 用在两个位置：登录 session 防滥用，以及频道消息防刷屏。它不是图片验证码，而是服务端要求客户端提交一个 6 字段 stamp，整个 stamp 做 SHA-256 后必须满足前导零 bit。

源码链路（`sneak/neoirc`）：

```text
session stamp = 1:bits:YYMMDD:serverName::counter_hex
channel stamp = 1:bits:YYMMDD:channel:sha256(raw_body_json):counter_hex
pass iff SHA256(stamp).leading_zero_bits >= bits
```

SDK 当前做法：

- session 模式可 GET `/api/v1/server` 读取 `name/hashcash_bits`，本地 mint `pow_token`，可 POST `/api/v1/session` 换 session token。
- channel 模式支持直接传 `channel + body/body_hash`，生成放在 message `meta.hashcash` 里的 body-bound stamp。
- counter 与官方前端一致使用十六进制字符串；支持固定 date 做可复现压测。
- 支持 artifact：`neoirc_run.json`。

命令示例：

```bash
antibot solve neoirc \
  --base-url 'https://chat.example' \
  --submit \
  --nick sdkbot
```

频道消息 body-bound fixture：

```bash
antibot solve neoirc \
  --challenge-json '{"mode":"channel","bits":16,"resource":"#pow","date":"260609","body_hash":"148f78d2d833b5a2b0162813b73187afb835e200d8499756c66d03107c5692c2"}' \
  --max-attempts 50000
```
...
```bash
antibot stress neoirc \
  --challenge-json '{"bits":12,"resource":"NeoIRC","date":"260609"}' \
  --runs 10 \
  --concurrency 2 \
  --max-attempts 20000
```
...
- 这是 resource/body 绑定的 Hashcash 协议 solver；服务端还有 spent stamp replay 防护，所以真实提交每个 stamp 只能用一次。
- channel 模式必须对齐服务端实际 `json.RawMessage body` 字节；如果业务 JSON 序列化顺序变了，bodyHash 会变。
...
# Powxy
antibot solve powxy --base-url 'https://target.example/protected' --submit --timeout 30 --max-attempts 5000000
...
# JustNoCaptcha
antibot solve justnocaptcha --challenge-url 'https://target.example/justnocaptcha/challenge'
...
# PowForge CAPTCHA
antibot solve powforge --base-url 'https://captcha.powforge.dev' --token-verify
antibot solve powforge --challenge-json '{"salt":"powforge-fixture-salt","difficulty":14,"algo":"sha256"}' --no-submit --timeout 5
antibot stress powforge --challenge-json '{"salt":"powforge-fixture-salt","difficulty":14,"algo":"sha256"}' --no-submit --runs 20 --concurrency 4
...
# NeoIRC Hashcash
antibot solve neoirc --base-url 'https://chat.example' --submit --nick sdkbot --max-attempts 100000000
...
src/antibot_sdk/
  ...
  providers/
    ...
    powforge.py             # PowForge signed salt SHA-256 PoW token solver
...
PowForge fixture/mock/live/CLI/stress: /api/challenge + SHA256(salt+nonce) leading-zero bit + /api/verify + /api/token/verify 验证通过
...
```text
go.lindenii.runxiyu.org/powxy@v0.1.86：handler.go / identifier.go / validate.go / static/solver.py / wasm/solver.c 交叉确认。
identifier = SHA256(go_varint10(week)||remote_ip||User-Agent||Accept-Encoding||Accept-Language||SHA256(privkey))。
PoW = SHA256(identifier||uint64_le(nonce)) 前 N bit 为 0；提交 powxy=base64(uint64_le(nonce))。
fixture：identifier=AAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8=, difficulty=20 -> nonce=96585, powxy=SXkBAAAAAAA=, digest=000001bd...。
...
```text
sneak/neoirc：internal/hashcash/hashcash.go + channel.go + web/src/app.jsx 交叉确认。
session stamp = 1:bits:YYMMDD:resource::counter_hex；channel stamp = 1:bits:YYMMDD:channel:bodyHash:counter_hex。
fixture：1:16:260609:NeoIRC::9510 -> digest=000029c1...；channel bodyHash=148f78d2... -> counter=33ee,digest=00009461...。
