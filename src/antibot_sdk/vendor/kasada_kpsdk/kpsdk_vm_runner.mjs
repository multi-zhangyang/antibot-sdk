...
class MiniStorage {
  constructor(init = {}) {
    this.map = new Map(Object.entries(init || {}).map(([k, v]) => [String(k), String(v)]));
  }
...
function makeSandbox(opts) {
  ...
  function dispatchEvent(event) {
    ...
    for (const cb of listeners.get(ev.type) || []) {
      try { cb.call(sandbox.window, ev); } catch (e) { events.push({ type: 'listener-error', source: ev.type, message: e && e.message }); }
    }
    return true;
  }
  ...
  async function nativeFetch(url, options = {}) {
    ...
    const headers = { ...requestHeaders, ...optionHeaders };
    const finalUrl = isRequest ? url.url : String(url);
    const item = {
      url: finalUrl,
      method: String(options.method || (isRequest ? url.method : '') || 'GET').toUpperCase(),
      headers,
      body: (options.body ?? (isRequest ? url.body : null)) == null ? null : String(options.body ?? url.body),
      at: Date.now(),
    };
    fetches.push(item);
    return {
  ...
  const sandbox = {
    ...
    navigator: {
      ...
      sendBeacon(url, data = null) {
        fetches.push({ url: String(url), method: 'POST', headers: {}, body: data == null ? null : String(data), at: Date.now(), beacon: true });
        return true;
      },
  ...
  sandbox.Image = class Image {
    ...
    set src(value) { this._src = String(value); if (typeof this.onload === 'function') setTimeout(() => this.onload(), 0); }
  ...
  sandbox.XMLHttpRequest = class XMLHttpRequest {
    ...
    send(body = null) {
      const item = { url: this.url || '', method: this.method || 'GET', headers: { ...this.headers }, body: body == null ? null : String(body), at: Date.now() };
      xhrs.push(item);
      this.readyState = 4;
