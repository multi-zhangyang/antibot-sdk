...
function normalize(input) {
  ...
  const scriptUrl = input.script_url || input.scriptUrl || new URL('/149e9513-01fa-4fb0-aad4-566afd725d1b/2d206a39-8ed7-437e-a3be-862e0f06eea3/p.js', pageUrl).href;
  return {
    script: String(input.script || ''),
    scriptUrl,
    pageUrl,
    requestUrl: input.request_url || input.requestUrl || '',
    requestMethod: String(input.request_method || input.requestMethod || 'GET').toUpperCase(),
    requestTransport: String(input.request_transport || input.requestTransport || 'fetch').toLowerCase(),
    requestHeaders: input.request_headers || input.requestHeaders || {},
    config: input.config || null,
    profile: input.profile && typeof input.profile === 'object' ? input.profile : {},
    settleMs: Number(input.settle_ms || input.settleMs || 50),
...
function makeSandbox(opts) {
  ...
  const cookieJar = makeCookieJar(profile.cookie || '');
  ...
  function capture(kind, url, options = {}) {
    ...
    const finalUrl = isRequest ? url.url : String(url);
    ...
    if (kind === 'xhr') xhrs.push(item);
    else fetches.push(item);
  ...
  function setTimeoutCompat(cb, delay = 0, ...args) {
    if (typeof cb !== 'function') return setTimeout(() => {}, Number(delay) || 0);
    return setTimeout(() => safeCall(cb, sandbox.window, args, 'setTimeout'), Number(delay) || 0);
  }
  function setIntervalCompat(cb, delay = 0, ...args) {
    if (typeof cb !== 'function') return setInterval(() => {}, Number(delay) || 0);
    return setInterval(() => safeCall(cb, sandbox.window, args, 'setInterval'), Number(delay) || 0);
  }
  function setImmediateCompat(cb, ...args) {
    if (typeof cb !== 'function') return (typeof setImmediate === 'function' ? setImmediate(() => {}) : setTimeout(() => {}, 0));
    return (typeof setImmediate === 'function' ? setImmediate(() => safeCall(cb, sandbox.window, args, 'setImmediate')) : setTimeoutCompat(cb, 0, ...args));
  }
  ...
  function appendDocumentChild(parent, child) {
    ...
    if (String(child.tagName || '').toLowerCase() === 'script' && !documentScripts.includes(child)) {
      documentScripts.push(child);
    }
    if (child.src) {
  ...
  function makeElement(tag) {
    ...
    const el = makeEventTarget({
      ...
      appendChild(child) { return appendDocumentChild(this, child); },
      removeChild(child) {
    ...
    Object.defineProperty(el, 'src', {
      ...
      set(value) {
        ...
        if (this.parentNode && lowered === 'script') {
          capture('script', this._src, { method: 'GET' });
          triggerElementLoad(this, 'script.src');
        }
      },
    });
  ...
  async function nativeFetch(url, options = {}) {
    ...
    return new MiniResponse('', { status: 204, statusText: 'No Content', url: item.url });
  }
  ...
  class MiniResponse {
    ...
    async text() { this.bodyUsed = true; return serializeBody(this._body) || ''; }
    async json() { return JSON.parse(await this.text()); }
    async arrayBuffer() { const buf = toBuffer(await this.text()); return buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength); }
  ...
  MiniXMLHttpRequest.UNSENT = 0;
  MiniXMLHttpRequest.OPENED = 1;
  MiniXMLHttpRequest.HEADERS_RECEIVED = 2;
  MiniXMLHttpRequest.LOADING = 3;
  ...
  class MiniMessagePort {
    constructor(label = 'message-port') {
      this.label = label;
      this.onmessage = null;
      this.closed = false;
      makeEventTarget(this);
    }
    postMessage(data) { if (!this.closed) messages.push({ data, origin: this.label, at: Date.now() }); }
    start() {}
    close() { this.closed = true; }
  }

  class MiniSharedWorker {
    constructor(url) {
      this.url = String(url || '');
      this.id = workerSeq++;
      this.onerror = null;
      this.port = new MiniMessagePort(`shared-worker:${this.id}`);
      makeEventTarget(this);
      capture('worker', this.url, { method: 'GET' });
    }
  }
  ...
  URLShim.createObjectURL = function createObjectURL(obj) {
    const url = `blob:${page.origin}/kpsdk-${blobSeq++}`;
  ...
  sandbox = {
    ...
    postMessage,
    fetch: nativeFetch,
    sendBeacon(url, data = null) { return sandbox.navigator.sendBeacon(url, data); },
    __nativeFetch: nativeFetch,
    __kpsdkMessages: messages,
    __kpsdkEvents: events,
    __kpsdkFetches: fetches,
    __kpsdkXhrs: xhrs,
    __kpsdkErrors: errors,
    __kpsdkObjectUrls: objectUrls,
  };
...
async function solve(input) {
  ...
  if (!opts.script.trim()) throw new Error('KPSDK VM requires non-empty script');
  ...
  vm.runInContext('window.window=window;window.self=window;window.globalThis=window;document.defaultView=window;', context, { timeout: 1000, filename: 'kpsdk-vm-prelude.js' });
  ...
  for (const ev of ['DOMContentLoaded', 'load', 'visibilitychange']) {
    try { context.dispatchEvent(new MiniEvent(ev)); } catch (e) { context.__kpsdkErrors.push({ type: 'dispatch-error', event: ev, message: e && e.message }); }
  }
  if (opts.config && context.KPSDK && typeof context.KPSDK.configure === 'function') {
    await context.KPSDK.configure(opts.config);
  }
  if (context.KPSDK && typeof context.KPSDK.isReady === 'function' && context.KPSDK.isReady()) {
    context.dispatchEvent(new MiniEvent('kpsdk-ready'));
  }
  await new Promise(resolve => setTimeout(resolve, Math.max(0, opts.settleMs)));

  let requestResult = null;
  if (opts.requestUrl) {
    const before = context.__kpsdkFetches.length;
    const xhrBefore = context.__kpsdkXhrs.length;
    try {
      if (opts.requestTransport === 'xhr') {
        const xhr = new context.XMLHttpRequest();
        xhr.open(opts.requestMethod, opts.requestUrl);
        for (const [key, value] of Object.entries(headersToObject(opts.requestHeaders))) {
          xhr.setRequestHeader(key, value);
        }
        xhr.send(null);
      } else {
        await context.fetch(opts.requestUrl, { method: opts.requestMethod, headers: opts.requestHeaders });
      }
    } catch (e) {
      requestResult = { error: e && e.message ? e.message : String(e) };
    }
    await new Promise(resolve => setTimeout(resolve, Math.max(0, opts.settleMs)));
    const newFetches = context.__kpsdkFetches.slice(before);
    const newXhrs = context.__kpsdkXhrs.slice(xhrBefore);
    requestResult = {
      ...(requestResult || {}),
      transport: opts.requestTransport,
      fetches: newFetches,
      xhrs: newXhrs,
      last: newFetches[newFetches.length - 1] || newXhrs[newXhrs.length - 1] || null,
    };
  }

  const kpsdk = context.KPSDK && typeof context.KPSDK === 'object' ? context.KPSDK : null;
  return {
    messages: context.__kpsdkMessages,
    events: context.__kpsdkEvents,
    fetches: context.__kpsdkFetches,
    xhrs: context.__kpsdkXhrs,
    errors: context.__kpsdkErrors,
    ...
    diagnostics: {
      ...
      kpsdkPresent: !!kpsdk,
      kpsdkReady: !!(kpsdk && typeof kpsdk.isReady === 'function' && kpsdk.isReady()),
      kpsdkKeys: kpsdk ? Object.keys(kpsdk).slice(0, 60) : [],
      messageCount: context.__kpsdkMessages.length,
      fetchCount: context.__kpsdkFetches.length,
      xhrCount: context.__kpsdkXhrs.length,
      eventCount: context.__kpsdkEvents.length,
      errorCount: context.__kpsdkErrors.length,
      objectUrlCount: context.__kpsdkObjectUrls.size,
    },
  };
}
